...
Der Bitcoin (BTC) ist eine sogenannte Kryptowährung, also eine digitale Währung, deren Besitzrechte und Transaktionen mittels kryptographischer Verfahren geregelt werden.
Während es hunderte verschiedener Währungen dieser Art gibt, ist der Bitcoin der bekannteste und verbreitetste.
Genau wie andere Währungen wird der Bitcoin gehandelt und mancherorts als Zahlungsmittel akzeptiert.
Der Wechselkurs liegt zum Zeitpunkt, an dem diese Ausarbeitung geschrieben wird, bei rund 5'700 EUR oder rund 6'500 USD für 1 BTC, ist jedoch teils starken Fluktuationen unterworfen.\footnote{https://bitcoinwisdom.com/}

Zahlungen per Bitcoin werden dezentral organisiert. Jeder, der die entsprechende Software ausführt, beteiligt sich an der Verifikation der Transaktionen.
Sämtliche Transaktionen, die verifiziert wurden, werden in der sogenannten ,,Blockchain`` hinterlegt, einem Logbuch, von dem jeder Teilnehmer eine lokale Kopie besitzt.
Die Blockchain ist eine einfach verkette Liste von Blöcken, von denen jeder neben Eckdaten zu seiner Erzeugung eine Auswahl von durch ihn verifizierte Transaktionen enthält.
Einzelne Blöcke unterliegen einer Größenbeschränkung von wenigen Megabyte. Die Vollständige Blockchain beansprucht momentan rund 180 Gigabyte.\footnote{https://www.blockchain.com/charts/blocks-size?}
Als Anreiz, sich an der Weiterführung dieser Kette zu beteiligen, wird dem Erzeuger eines Blocks ein bestimmter pauschaler Betrag an Bitcoins gutgeschrieben, zusätzlich zu den Kosten der verifizierten Transaktionen.
Dieser Pauschalbetrag begann mit einem Wert von 50 BTC, und wird nach der Erzeugung von jeweils 210'000 Blöcken halbiert. Der aktuelle Bonus liegt bei 12,5 BTC, die nächste Halbierung wird im zweiten Quartal 2020 erwartet.

Der Grund, dass die Generierung der Blöcke derart langsam voranschreitet, ist, dass sie künstlich erschwert wird.
Zur Generierung eines gültigen Blocks ist es notwendig, dass der Hashwert dieses Blocks unterhalb eines gewissen Schwellwerts liegt.
Der Schwellwert wird alle 2016 Blöcke neu berechnet, und zielt darauf ab, dass weltweit durchschnittlich nur ein gültiger Block alle 10 Minuten entsteht.
Anstatt des Schwellwerts selbst wird die sogenannte ,,Schwierigkeit`` nach außen kommuniziert, ein Wert, der sich umgekehrt proportional zum Schwellwert verhält.
Innerhalb des letzten Jahres ist der Schwierigkeitsgrad insgesamt um etwa den Faktor 6,6 gestiegen.
Zuletzt wurde er am 4. Oktober 2018 auf rund $7,45 \cdot 10^{12}$ festgelegt, basierend darauf, dass die beteiligten Systeme seit der vorigen Festlegung insgesamt rund $5,3 \cdot 10^{19}$ Hashwerte pro Sekunde errechneten.\footnote{https://bitcoinwisdom.com/bitcoin/difficulty}

Insgesamt lässt sich die Unternehmung, die Blockchain zu erweitern, mit einer Tombola vergleichen.
Durch die Verwendung von Hashfunktionen ist es effektiv Zufall, ob man einen gültigen Block erzeugt, oder nicht.
Die Chancen, den Gewinn zu ziehen, hängen dann nur noch davon ab, wie oft man in Topf greift.
Ohne erheblichen Aufwand ist es höchst unwahrscheinlich, einen Treffer zu landen.
