...
Der Programmcode ist frei verfügbar, inklusive Demonstrationsvideos. \footnote{https://github.com/ratnim/SelfawareMonopoly}
\end{abstract}
...
\begin{figure}[h]
\caption{Beginn eines Monopolyspiels}
\includegraphics[width=8cm]{gameboard}
\end{figure}

\section{Konzept}\label{sec:concept}

Als das Spiel Monopoly in seiner aursprünglichen Form 1904 das erste mal aufkam, war es als sozial-kritisches Spiel gedacht. \footnote{https://winningmoves.de/blog/die-geschichte-des-monopoly-spiels, Stand: 01.01.2019}
Aufbauend auf dieser Grundidee wollen wir nun ein Spiel entwerfen, welches allen Spielern die Möglichkeit gibt die Spielregeln zu umgehen und sich im Gegenzug Information und Resourcen von den Spielern beschaft.
Diese Komponente wird im folgenden \neuerBegriff{Watson} genannt.

\subsection{Die Watson-Komponente}

Watson bietet dem Spieler immer wieder die Möglichkeit, die Regeln von Monopoly zu umgehen und dem Spieler so einen Vorteil zu verschaffen.
...
Sollten alle Spieler eines Spiels zu keiner Zeit die Deals von Watson annehmen, entspricht das Spiel einer regelkonformen Monopolypartie.

Der Name Watson ist frei gewählt und wurde von uns bewusst als Personifikation eines Algorithmuses gewählt. Assoziationen zu IBM-Watson, dem Supercomputer, der souverän Quizfragen beantwortet 
%\footnote{https://www.ibm.com/midmarket/us/en/article_Smartercomm5_1209.html} 
und Sherlock Homes Assistenten Watson sind vielleicht keine Zufälle. 

\subsection{Watsons' Ziel}

Watson interessieren persöhnliche Informationen, Ressourcen und die Nutzung der Identität der Spieler, um in ihrem Namen aufzutreten.

Persöhnliche Informationen, die Watson bekommen möchte, sind zum Beispiel die Identität des Spielers auf Sozialen Platformen oder Zugriff auf andere Dienste wie E-Mail Services.

Ressourcen die Watson bekommen möchte ist Rechnenleistung um Bitcoinmining zu betreiben.

Auf welche Dienste dabei zurückgegriffen wird, wird in \ref{sec:whatwatson} erörtert.

\subsection{Angebotene Deals}

\begin{figure}[h]
\caption{Konzept visualisiert: Abwägen zwischen Mehrwert im Spiel und Preigeben von Informationen und Ressourcen.}
\includegraphics[width=0.5\textwidth]{spiellogik}
\end{figure}

Watson bietet dem Spieler unterscheidliche Deals an, um die Regeln zu umgehen.
...
\hline
\end{tabular}
\caption{Die Tabelle kategorisiert die Effekte der Deals, die durch Watson angeboten werden, in zwei Dimensionen. In der horizontalen, ob der Effekt eines Deals für die Spieler sichtbar ist und in der vertikalen, ob der Effekt direkt eintritt.}
\label{tab:effekte}
\end{table}

Tabelle \ref{tab:effekte} kategorisiert die Effekte eines Deals in zwei Dimensionen. \textit{Direkt} oder \textit{indirekt}, das bedeutet ob der Spieler einen direkten Nutzen bekommt und in \textit{verdeckt} und \textit{offensichtlich}, was bedeutet, ob die anderen Mitspieler mitbekommen, ob soeben ein Deal gegriffen hat.
...
Der Spieler weiß zu den Zeitpunkt, wenn ihm der Deal angeboten wird nicht, ob er die Leistung durch Watson braucht.
...
Baut ein Spieler, der Watson-Coins besitzt, Häuser, so wird der Preis pro Watson-Coin um 50 Euro verringert.
...
Watson bietet deshalb den Spielern an die Würfel zu manipulieren.
Der Spieler muss dafür persönliche Daten an Watson übergeben und darf sich so sein nächstes Würfelergebniss wählen.
Für andere Spieler ist es in keiner Weise ersichtig, ob der Spieler einen Deal von Watson angenommen hat oder nicht.
...
Tut der Spieler dies, merkt sich Watson dies und beeinflusst die Ereignisskarten, die ein Spieler bekommt, wenn er auf ein Ereignissfeld läuft.


\subsection{Timing des Eingreifens}

Es ist wichtig das Watson für andere Spieler nicht merklich in den Spielfluss eingreift.
Das heißt, wenn ein Spieler mit Watson interagiert sollen es die anderen Spieler nicht mitbekommen.
Dafür sollte Watson möglichst dann die Deals anbieten, wenn der Spieler gerade im Spiel nichts zu tun hat.
Deshalb bietet Watson Deals eher Spielern an, die gerade nicht an der Reihe sind.

Desweiteren soll Watson eine hohe Erfolgsrate haben, das heißt, die Wahrscheinlichkeit, das ein Spieler den Deal annimmt soll hoch sein.
...
Beispiele: Einem Spieler der sehr viele Straßen besitzt, anzubieten die Würfel zu manipulieren hat wahrscheinlich weniger Erfolgschancen, als einem Spieler der unbedingt diese eine Straße braucht.
...
Wohingegen Spieler die noch keine Deals angenommen haben weniger bis garkeine Angebote mehr bekommen.
...
\subsection{Watson-Spieler Interaktion}

\begin{figure}[h]
\caption{Watson Dialog: Beeinflussen der Würfel}
\includegraphics[width=0.5\textwidth]{watson_dices}
\end{figure}

\begin{figure}
\caption{Weiterleitung zum OAuth Dialog des API Anbieters. In diesem Fall werden Berechtigungen von GMail eingefordert.}
\includegraphics[width=0.5\textwidth]{gmail_access}
\end{figure}
...
Watson erscheint als ein Dialogfenster. Dabei gibt es in der Eingabemaske dem Spieler zunächst den Vorteil bekannt, sowie mögliche Konfigurationsparameter, wie z. B. Würfelergebnis. Darunter folgt die Aufforderung eine Aktion auszuführen. Die Aktion ist in der Regel mit einem Klick ausführbar.

\subsection{Arten von Deals}\label{sec:whatwatson}


Für die Aktionen haben wir auf Dienste von Drittanbietern konzentriert, die APIs bereitstellen, welche persönliche Informationen oder Aktionen zulassen. Folgende Dienste haben wir in Betracht gezogen: Amazon, Twitter, Facebook,  Instagram, LinkedIn, Spotify, Google Mail und PayPal.
Eine Analyse der API Dpkumentationen hat folgendes ergeben. Amazon hat keine für uns nützliche API. LinkedIn bietet lediglich den Usernamen. Bei allen anderen Diensten ist es möglich im Namen des Spielers, sofern er die Berechtigung freigegeben hat, zu handeln.
Um Zugriffsrechte zu bekommen wird die von allen Diensten angebotene OAuth2.0 Schnittstelle verwendet. OAuth ist ein Prozess bei dem sich ein Nutzer bei einem Dienst anmeldet und anschließend ein Zugangsschlüssel (sog. \textit{access token}) bereitgestellt wird, welcher mit ausgewählten Berechtigungen versehen ist. Mit diesem Zugangsschlüssel kann die authorisierte Anwendung dann Daten abfragen oder Aktionen ausführen.

Im Abschnitt \ref{sec:apidiscussion} gehen wir genauer auf die Informationen und Aktionen die abgegriffen bzw. ausgeführt werden können ein und wie Nutzer auch dem entgegen wirken können.

Ein weiterer Nutzen neben den Informationen sind Ressourcen. Hierfür haben wir uns vor allem auf das Thema Bitcoinmining konzentriert. Beim Bitcoinmining wird die Kryptowhärung Monero gemined. Dafür wird die Coinhive API verwendet, welche Bitcoint-Mining im Browser ermöglicht. \footnote{https://coinhive.com/}
Das tatsächlich erwirschaftete Guthaben geht an Watson, bzw. an das System, bzw. an die Autoren dieses Spiels.
...
\begin{figure}[h]
\caption{Softwarearchitektur visualisiert}
...
\section{Diskussion}\label{sec:discussion}\label{sec:apidiscussion}

Wie bereits in \ref{sec:whatwatson} angesprochen, macht sich die Watson Komponente die APIs verschiedener Dienste zunutze. In diesem Abschnitt wird diskutiert, welche Rechte über die APIs freigegeben werden.

Durch den Prototypen wurde gezeigt, das es möglich ist Systeme zu bauen, die versuchen den Nutzer mit seiner Zustimmung dazu zu bringen Information und Ressourcen von sich herzugeben ohne einen Mehrwert außerhalb des bereitgestellten Spiels zu gewähren.
...
Facebook ist mit über 2000 Millionen Nutzern \footnote{laut Facebook Report} die größte soziale online Platform. Sie ermöglicht den Austausch mit Freunden und Fremden. Als Entwickler kann man sog. Apps für Facebook schreiben. Folgende Informationen können dabei erfragt werden : E-Mail, Name, Alter, Geschlecht, Geburtstdatum, Likes und weitere
Außerdem ist es möglich einer Seiten einen sog. \textit{Like} zu geben.
Folgende Informationen und Aktionen sind u.a. nicht möglich: Freundesliste abfragen und persönliche Nachricht schreiben.
...
\section{Analogie zu unserer Gesellschaft}
...
\subsection{Verdeckte-Direkte Deals in der Gesellschaft}
...
\subsection{Offene-Indirekte Deals in der Gesellschaft}
...
\subsection{Verdeckte-Indirekte Deals in der Gesellschaft}
...
\section{Warum baut man solche Systeme?}
...
Andere Spiel wie Abstraction aber auch Anwendungen wie Sweat-Coin nutzen Bitcoin-Mining um mit der Rechenleistung des Spielers Kapital zu generieren.
...
\begin{figure}
\caption{Verlauf des Spiels inklusive Auswirkungen des Coin-Minings.}
