...
TEST(IdleStateTest, buy_house_not_enough_money)
{
    Game game(fieldsTwoGroups());
    game.players() = RingBuffer<Player>(std::vector<Player>{ { player_1 } });
    game.stateChange<IdleState>();
    ...
    EXPECT_THROW(game.requestBuyHouse(player_1, 0, true), Exception);
...
TEST(IdleStateTest, buy_house_not_owner)
{
    Game game(fieldsTwoGroups());
    game.players() = RingBuffer<Player>(std::vector<Player>{ { player_1 } });
    game.stateChange<IdleState>();
    ...
    EXPECT_THROW(game.requestBuyHouse(player_1, 2, true), Exception);
...
TEST(IdleStateTest, buy_house_full)
{
    Game game(fieldsTwoGroups());
    game.players() = RingBuffer<Player>(std::vector<Player>{ { player_1 } });
    game.stateChange<IdleState>();
    ...
    dynamic_cast<Street*>(game.board()[0])->changeOwner(player_1);
    dynamic_cast<Street*>(game.board()[0])->changeConstructionLevel(ConstructionLevel::HOTEL);
    dynamic_cast<Street*>(game.board()[1])->changeOwner(player_1);
...
TEST(IdleStateTest, sell_house)
{
    ...
    game.requestBuyHouse(player_1, 0, false);
...
TEST(IdleStateTest, sell_house_not_owner)
{
    Game game(fieldsTwoGroups());
    game.players() = RingBuffer<Player>(std::vector<Player>{ { player_1 } });
    game.stateChange<IdleState>();
    ...
    EXPECT_THROW(game.requestBuyHouse(player_1, 2, false), Exception);
...
TEST(IdleStateTest, sell_house_empty)
{
    Game game(fieldsTwoGroups());
    game.players() = RingBuffer<Player>(std::vector<Player>{ { player_1 } });
    game.stateChange<IdleState>();
    ...
    EXPECT_THROW(game.requestBuyHouse(player_1, 0, false), Exception);
